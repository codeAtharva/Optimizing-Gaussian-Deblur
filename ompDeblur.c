...
*/
		...
		for(z = 1; z < zMax; z++)
		{
			for(x = xMax -1; x--;)
			{
				for(y = yMax -1; y--;)
				{
					int temp = ((z * yMax + y) * xMax + x);
					u[temp] = u[Index(x, y, z -1)] ;
...
void OMP_Deblur(double* u, const double* f, int maxIterations, double dt, double gamma, double sigma, double Ksigma)
{
	...
	//	#pragma omp parallel for private(y,z)
			for(y = 1; y < yMax - 1; y++)
			{
				...
				{	
					int temp = ((z * yMax + y) * xMax + x);
					g[temp] = 1.0 / pow(epsilon + 
						SQR(u[temp] - u[temp + 1]) + 
						SQR(u[temp] - u[temp - 1]) + 
						SQR(u[temp] - u[temp + xMax]) + 
						SQR(u[temp] - u[temp - xMax]) + 
						SQR(u[temp] - u[temp + xy]) + 
						SQR(u[temp] - u[temp - xy]), 0.5);
				}
			}
		}

		memcpy(conv, u, sizeof(double) * xyz);
...
*/
		...
		for(z = 1; z < zMax - 1; z++)
		{
			for(x = 1; x < xMax - 1; x++)
			{
				...
				{
					int temp = ((z * yMax + y) * xMax + x);
					double oldVal = u[temp];		
						
						double newVal = (u[temp] + dt * ( 
						u[temp - 1] * g[temp - 1] + 
						u[temp + 1] * g[temp + 1] + 
						u[temp - xMax] * g[temp - xMax] + 
						u[temp + xMax] * g[temp + xMax] + 
						u[temp - xy] * g[temp - xy] + 
						u[temp + xy] * g[temp + xy] - gamma * conv[temp])) /
